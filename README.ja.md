...
Hybrid Dev Orchestrator（HDO）は、GitHub Issue を実装契約へ正規化し、専用 Git worktree で planning、implementation、validation、review、fix を有限回実行する Windows CLI です。
...
- Windows 11
- Node.js 24 LTS と、checkout での `npm ci` 実行（1回）。[ADR-0001](docs/adr/0001-primary-runtime-typescript.md) による段階移行の結果、CLI と両 plugin は現在 node 上で動作します
- PowerShell 7.2 以上（`pwsh`）は、フェーズ8 完了以降は **任意** です。TypeScript runtime のどの route も `pwsh` を要求しません。PowerShell 側の回帰テストを実行する場合、maintenance mode の `hdo.ps1` を使う場合、または config が PowerShell worker を明示的に起動する場合にのみ必要です（Windows 同梱の Windows PowerShell 5.1 では動作しないため、別途導入してください）
- WSL2 / Linux は正式な動作対象ではありません。移行の一次ターゲットは Windows です
- Git for Windows
...
- PowerShell 実装は、Migration strategy フェーズ6・フェーズ7 両方の終了条件を満たしたことを受け **maintenance mode（既存不具合の修正のみ、新規 subsystem は追加しない）へ移行しました**。`hdo.ps1` と `src/HybridDevOrchestrator/` は削除していません。`workers/hdo-ollama-worker.ps1` も **フェーズ8の完了（2026-09-07）をもって maintenance mode へ移行しました**。これにより `pwsh` は TypeScript runtime のどの route の要件でもなくなりました（この PowerShell worker を明示的に起動し続ける独自 config を使う場合に限り、その環境に `pwsh` が必要です）。フェーズ8の (c) - inner tool loop に AI SDK を採用するかの判断 - は実測の結果 **不採用**（依存 0 ベースライン継続）で決着しています（`docs/adr/0003-agent-harness-lightweight.md` Amendment 2026-09-07、実測データは `poc/ai-sdk/results/`）。移行の詳細な配置・依存方向は `docs/architecture.md` 16節を参照してください。
...
開発環境の準備、test suite の実行方法、plugin version bump の要件は
[CONTRIBUTING.md](CONTRIBUTING.md) を参照してください。本プロジェクトは
[Contributor Covenant](CODE_OF_CONDUCT.md) に従います。
