...
// process access, so they do not violate the core/platform boundary). Ported from
// poc/typescript/src/core/contracts/validate.ts, unchanged.
//
// Ajv options note: the canonical schemas under `schemas/` were authored for
// PowerShell's `Test-Json` (a .NET/Newtonsoft-based validator) and are NOT modified
// for this implementation. Constructing Ajv with default `strict: true` throws at
// compile time on those schemas (e.g. unevaluated `if`/`then` siblings and numeric
// keyword combinations that Ajv's strict mode flags as likely mistakes even though
// they are valid, intentional JSON Schema 2020-12). We therefore construct Ajv with
