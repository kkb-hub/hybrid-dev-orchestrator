...
3. Resolve `../../src/cli/main.ts` relative to this skill directory to an absolute path. Keep the target repository as the working directory. If the command fails because `node_modules` is missing, run `npm ci` once in the plugin root, then retry.
4. Run `node <absolute-path> run -Json` with the explicitly selected Issue or pickup mode and only additional HDO options the user supplied, such as `-Repository`, `-Profile`, `-Config`, `-SetStep`, `-IgnoreRepositoryConfig`, `-DryRun`, or `-NoWriteBack`. Do not add `-Config` merely because the repository has `.hdo/config.json`; HDO automatically reads its committed `HEAD` version.
