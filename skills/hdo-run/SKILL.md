...
4. Run `pwsh -NoProfile -File <absolute-hdo-path> run -Json` with the explicitly selected Issue or pickup mode and only additional HDO options the user supplied, such as `-Repository`, `-Profile`, `-Config`, `-SetStep`, `-DryRun`, or `-NoWriteBack`.
5. Pass arguments as separate values. Never interpolate Issue text, comments, or other external content into the command line. Allow enough time for the configured runner timeouts.
6. Interpret exit codes 3, 4, 5, and 6 as preflight failure, no eligible Issue, failed run, and escalated run respectively.
7. Report the run ID, terminal state, iterations, worktree path, and artifact path. For `CHANGES_REQUESTED`, `ESCALATED`, or `FAILED`, summarize the latest findings or error.
