...
フェーズ6の outer workflow の書き方（自作 dispatch loop、XState は採らない、ADR-0003 D1）と、フェーズ8での lean worker 移植の進め方（依存 0 ベースライン → `poc/ai-sdk/` 比較 PoC → 採否記録）は ADR-0003（`docs/adr/0003-agent-harness-lightweight.md`）に記録した。フェーズ8の終了条件 (iii)（下記「フェーズ8」段落参照）が完了した現在、既定の `config/examples/ollama-lean-worker.json` は `node` 版 lean worker を起動するため `pwsh` を必要としない。フェーズ8全体はまだ完了していないため、`workers/hdo-ollama-worker.ps1`（PowerShell worker）を明示的に起動し続ける構成に対しては `pwsh` が引き続き必要である。
...
**フェーズ8（workers）は (a)・(i)・(ii)・(iii) まで完了した（2026-09-07）。(iv) は未了であり、フェーズ8全体はまだ完了していない。** 終了条件（ADR-0001 Migration strategy フェーズ8、ADR-0003 D2）は次の4つである。
...
**(iv)**（`poc/ai-sdk/` への AI SDK 版比較 PoC の追加と、Issue #48 の指標に基づく採否決定の ADR-0003 Amendment または新 ADR への記録）は本 PR のスコープに含めておらず、後続 PR に委譲する。ADR-0001 Migration strategy フェーズ8および ADR-0003 D2 が定める「(i)〜(iv) を満たした時点で `pwsh` を TypeScript runtime の route 2 要件から外し、PowerShell worker は maintenance mode へ移行する」という条件はまだ発効していない: `workers/hdo-ollama-worker.ps1` は (iv) の完了まで live のまま残る唯一のコンポーネントである。(iii) により既定の `config/examples/ollama-lean-worker.json` はもう `pwsh` を必要としないが、この PowerShell worker を明示的に起動し続ける構成（`command`/`args` を差し替えていない旧来の config）に対しては `pwsh` が引き続き必要である。次のフェーズはこの (iv) を扱う後続 PR である。
...
以下はフェーズ8（workers、lean worker の TypeScript 移植）で判明した追加の意図的な差異である。(a)・(i)・(ii)・(iii) は完了しているが、(iv) は未了であり `workers/hdo-ollama-worker.ps1` は現在も PS oracle として live である点は §16.2「フェーズ8」段落を参照。
