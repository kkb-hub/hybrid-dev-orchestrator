// Mechanically enforces the core/platform boundary described in the PoC's package
// layout: nothing under `src/core/**` (excluding all `*.test.ts` files under core,
// this file included) may import a host-specific module or reach "up and out" into
// platform/process/git/cli. Every `*.test.ts` file under core is exempted from its own
// rule by `listSourceFiles`'s `.test.ts` filter below: a *test* that inspects source
// text is allowed to use `node:fs`/`node:path` to do so, because it never ships as
// part of the pure core surface that the rest of the program depends on.
//
...
test("findBoundaryViolations detects every bypass form on synthetic snippets (self-test, not files)", () => {
  ...
  const legitimate = `import { redactSecrets } from "../process/redact.ts";`;
  assert.deepEqual(
